...
import { SPRINT_STATE_VERSION, type ArtifactRecord, type ArtifactSink, type SprintState } from "./types.ts";
...
async function assertNoSymlinkSegments(base: string, target: string, includeTarget = true): Promise<void> {
...
export async function atomicCreateFile(path: string, content: string): Promise<void> {
	...
	const target = await stat(path);
	if (target) throw new Error(`Publication target already exists: ${path}`);
	const temporary = join(directory, `.${basename(path)}.${process.pid}.${randomUUID()}.tmp`);
	try {
		await writeFile(temporary, content, { encoding: "utf8", flag: "wx" });
...
export class RunArtifactStore implements ArtifactSink {
	...
	async replaceFlatDirectory(relativeDirectory: string, files: readonly { path: string; content: string }[]): Promise<ArtifactRecord[]> {
		await this.#assertRoot();
		const directory = assertSafeRelativePath(relativeDirectory);
		if (directory.includes("/")) throw new Error("Replacement directory must be one direct child of the run.");
		const target = resolve(this.runDirectory, directory);
		for (const name of await readdir(this.runDirectory)) {
			if (!name.startsWith(`.${directory}.`) || (!name.endsWith(".tmp") && !name.endsWith(".old"))) continue;
			const stale = resolve(this.runDirectory, name);
			const staleEntry = await stat(stale);
			if (!staleEntry?.isDirectory() || staleEntry.isSymbolicLink()) throw new Error(`Unsafe stale replacement directory: ${name}`);
			await rm(stale, { recursive: true });
		}
		const token = randomUUID();
		const temporary = resolve(this.runDirectory, `.${directory}.${token}.tmp`);
		const backup = resolve(this.runDirectory, `.${directory}.${token}.old`);
		await mkdir(temporary);
		const records: ArtifactRecord[] = [];
		try {
			for (const file of files) {
				const selected = assertSafeRelativePath(file.path);
				if (selected.includes("/")) throw new Error("Replacement files must be flat.");
				const content = file.content.endsWith("\n") ? file.content : `${file.content}\n`;
				await writeFile(resolve(temporary, selected), content, { encoding: "utf8", flag: "wx" });
				records.push({ path: `${directory}/${selected}`, sha256: sha256(content), bytes: Buffer.byteLength(content) });
			}
			const current = await stat(target);
			if (current) {
				if (!current.isDirectory() || current.isSymbolicLink()) throw new Error(`Replacement target is not a regular directory: ${directory}`);
				await rename(target, backup);
			}
			try {
				await rename(temporary, target);
			} catch (error) {
				if (await stat(backup)) await rename(backup, target).catch(() => undefined);
				throw error;
			}
...
export async function publishDirectoryAtomically(
	...
	const parentEntry = await stat(resolve(parent));
	if (!parentEntry?.isDirectory() || parentEntry.isSymbolicLink()) throw new Error("Publication parent is not a regular directory.");
	const target = resolve(parent, selectedName);
	assertInside(parent, target);
	if (await stat(target)) throw new Error(`Publication target already exists: ${target}`);
	const temporary = resolve(parent, `.${selectedName}.${randomUUID()}.tmp`);
	await mkdir(temporary);
	try {
		for (const file of files) {
