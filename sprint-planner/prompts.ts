...
A phase is the atomic unit for one head-down implementation agent. Group cohesive edits by target, domain, or vertical behavior — do not create one agent per bullet or aspect. Each unsplit phase must be a coherent, self-contained unit an implementer can own end-to-end and should reasonably fit within one agent session, using a practical assumed maximum of roughly 200,000–300,000 tokens. Use planning judgment only; do not perform or print a formal token estimate.

If a cohesive phase would be overly large for one session, divide its Implementation Steps into contiguous lettered subphases (A, B, C, and so on). Each subphase maps to its own sequential DeepSeek implementation agent session — do NOT write boilerplate claiming all subphases are one agent or one unit. Make each subphase a granular, ordered unit of work that should itself fit within one agent session under the same practical assumption. Keep the parent phase as the validation unit: phase validation happens only after every lettered subphase is complete, with no independent validation gate between subphases. The phase must explicitly instruct the implementer that each subphase is a separate agent session executed in letter order before the shared phase validator runs.
