...
- Human model labels must be translated to canonical Pi tuples where known: DeepSeek Pro V4 is `deepseek/deepseek-v4-pro`. The orchestration validator tuple is `openai-codex/gpt-5.6-terra:high`. Exact tuples still fail rather than silently substituting another model.
