...
- Human model labels must be translated to canonical Pi tuples where known: DeepSeek Pro V4 is `deepseek/deepseek-v4-pro`. The orchestration validator tuple is `openai-codex/gpt-5.6-sol:medium`; the earlier GPT-5.5 reference was a user-confirmed typo. Exact tuples still fail rather than silently substituting another model.
