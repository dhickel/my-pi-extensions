...
Each phase groups cohesive edits by targets, domain, or vertical logic and is executable by exactly one implementation agent; steps and aspects are instructions within that delegation, not additional workers. A phase provides head-down guidance: exact targets, ordered edits, invariants, edge cases, and only concise code or pseudocode examples that materially reduce ambiguity. It omits obvious background, repetition, speculative detail, and other context bloat.
