...
All five agent-callable tools are sequential long-running tools. `sprint_ironout` and `sprint_advanceplan` expose no model parameters: the engine supplies the loaded configuration's `ironoutAuthor` and `ironoutReviewer` assignments, while advanced planning uses the configuration loaded from `sprint-planner/configs/default.ts`. Relative input paths resolve from Pi's current working directory; a leading `@` is accepted for parity with built-in path tools.
...
Brainstorm, ironout, and advanced-planning tuples are exact assignments in the schema-conforming configuration files under `sprint-planner/configs/`, selecting reusable `MODEL_PROFILES` tuples from `types.ts`. `configs/default.ts` is registered as the sole default, and extension initialization loads it once before supplying the same snapshot to every engine. No caller or runtime selection exists yet.

All extension model tuples are exact:

| Responsibility | Provider/model | Thinking |
| --- | --- | --- |
| Brainstorm role routing | `openai-codex/gpt-5.6-luna` | `xhigh` |
| Brainstorm and cross-review workers | `deepseek/deepseek-v4-pro` | `max` |
| Brainstorm synthesis | `openai-codex/gpt-5.6-luna` | `xhigh` |
| Brainstorm red team | `openai-codex/gpt-5.6-sol` | `high` |
| Ironout author / corrective reviewer | `openai-codex/gpt-5.6-sol` / `openai-codex/gpt-5.6-luna` | `high` / `xhigh` |
| Advanced planner / adviser | `openai-codex/gpt-5.6-sol` | `high` / `max` |
| Advanced decomposition, concepts, orchestration, and phase reviewers | `openai-codex/gpt-5.6-luna` | `xhigh` |
...
Brainstorming produces implementation approaches for the user's directive. Approaches may include some quality-of-life and supporting features that genuinely serve the directive, but they must not balloon or expand scope: workers tighten and refine the request while weighing options and concerns, and avoid out-of-domain features and work.
...
The author produces a high-level handoff covering context, objective, targets, features, settled decisions, constraints, scope, assumptions, direction, validation, open questions, and sign-off. One corrective reviewer at the loaded configuration's `ironoutReviewer` assignment returns a complete corrected handoff plus its review. Full sprint ironout is autonomous; standalone interactive ironout uses the root-context `user-questioning` service for at most three rounds of at most three choice questions.

In full sprints the author receives the user's original directive (or the handoff provided to brainstorming) as the authoritative scope contract, together with the brainstorm synthesis and red-team material as supporting implementation approaches. The corrected handoff must complete all feature functionality requested by the user and exclude out-of-domain features and work. Never-defer applies to user-directive work and decided-on features: all such work must be completed fully unless stubs or deferred items are part of the user directive itself.

## Advanced Plan Contract

The planner may make at most two `max` adviser calls for genuinely advanced or blocked planning areas. It submits exactly one `concepts.md`, one `orchestration.md`, and a scope-budgeted set of contiguous flat `phase-NN-<slug>.md` files — small 2–3, medium 3–5, large 6–10, or extra-large 11–20 phases. Most sprints should normally land around 5–10 phases; lettered subphases and plans up to 20 phases are supported when the work genuinely requires them. Planning makes the complete plan and does not trim work to save tokens. A plan contains no review or runtime entries.

Each unsplit phase groups cohesive edits by targets, domain, or vertical logic and should reasonably fit within one implementation-agent session under the planner's practical assumption of roughly 200,000–300,000 tokens maximum; this is a judgment call, not a formal calculation or printed estimate. Plans cover the requested and required scope only: production quality means fully implementing and validating each requested behavior, not adding speculative, adjacent, optional, or otherwise unrequested features to make the codebase appear feature-complete or production-ready. Mocks, stubs, placeholders, deferred work, partial implementations, and bare-minimum shortcuts are acceptable only when the user directive itself asks for them; every user-requested behavior and every feature decided on during planning must be completed fully. When cohesive work would make a phase overly large, its Implementation Steps use contiguous lettered subphases (A, B, C, and so on). Each subphase is a granular one-session implementation unit, while the parent phase remains the dependency and validation unit: subphases execute in letter order and phase validation happens only after all of them complete. Ordinary steps and aspects remain instructions within one delegation rather than additional workers. Every phase provides head-down guidance: exact targets, ordered edits, invariants, edge cases, and only concise code or pseudocode examples that materially reduce ambiguity. It omits obvious background, repetition, speculative detail, and other context bloat.
...
Advanced planning resolves the loaded configuration and hard-codes the model contract into each generated plan. The skill consumes the plan-owned contract: `basicImplementer` for `basic`-labelled phases, `advancedImplementer` for `advanced`-labelled phases, and the plan's validation tuple for every phase validator and final integration. Only when the authoritative input carries no validated model assignments — raw prose, checklists, pasted legacy plans, or single plan files — does the skill fall back to the active configuration's `basicImplementer`, `advancedImplementer`, `phaseValidator`, and `seniorAgent` assignments.

The skill requires Pi's subagent spawn, poll, status, and cancel tools. Before edits, one atomic spawn batch preflights every distinct assigned implementation tuple plus the plan-owned validation tuple with no-op agents. Missing models, authentication, tools, thinking support, or an unreadable configuration stop execution; the skill never substitutes another tuple. One unsplit phase maps to one implementation-agent session at its validated plan assignment. A phase with explicit lettered subphases maps each subphase to one sequential session at the parent phase's assignment in letter order; no runtime token calculation or enforcement is performed.

Every implementation wave fully settles before validation. Every phase receives one independent review-and-repair agent at the plan-owned validation tuple with full edit authority only after its unsplit implementation or all of its lettered subphases complete. The validator inspects actual repository state, checks every criterion, repairs in-scope defects itself, reruns required checks, and returns exactly `VERDICT: PASS` or `VERDICT: BLOCKED` with evidence sections. There is no read-only validation mode, `VERDICT: REPAIR`, or separate DeepSeek repair handoff. A malformed or missing verdict is retried once, then blocks. A valid `BLOCKED` verdict is a durable retryable attempt: disjoint siblings may continue, the same phase may receive later validation attempts, and dependents wait until every dependency's latest verdict is `PASS`. After all phases pass, one integration validator at the plan-owned validation tuple performs the same review-and-repair gate across the complete workflow.
